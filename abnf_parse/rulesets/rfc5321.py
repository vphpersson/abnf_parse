...
RFC5321_RULESET: Ruleset = Ruleset({
    ...
    'dot-atom-text': RFC5322_RULESET['dot-atom-text'],
...
# Based on observation, the following could be changed to match more `Received` headers/`Stamp`s in use:
# - In `ID`, replace `Atom` with `Dot-string`.
# - In `Extended-Domain`, add `address-literal`.
# - In `Stamp`, make `From-domain` optional.
